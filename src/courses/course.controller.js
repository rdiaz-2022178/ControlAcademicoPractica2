...
export const update = async (req, res) => {
    try {
        let { id } = req.params
        let data = req.body
        let userId = req.body; // Suponiendo que tienes la información del usuario en req.user.id
        ...
        let course = await Course.findOne({ _id: id, teacher: userId });
    ...
    export const deleted = async (req, res) => {
        try {
            ...
            // Verifica si el usuario es el propietario del curso
            const course = await Course.findOne({ _id: id, teacher: userId });
